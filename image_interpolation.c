...
void interpolate_forward_2d_double(double *Iin, double *Tlocalx, double *Tlocaly, int *Isize, int *ImageSize, double *Iout)
{
    ...
    for(jy=0; jy<Isize[1]; jy++)
    {
        for(jx=0; jx<Isize[0]; jx++)
        {
            ...
            jxmin=std::max(jx-1,0);
            jymin=std::max(jy-1,0);
            jxmax=std::min(jx+1,Isize[0]-1);
            jymax=std::min(jy+1,Isize[1]-1);
            ...
            b=(int)ceil(std::max(1.5*std::max(std::max(std::max(b1,b2),b3),b4),1.0));
            bd=(double)b;
            bc=-(1.0/(bd*bd))/0.04;
    ...
    for(i=0; i<ImageSize[2]; i++) /* Loop RGB */
    {
        ...
        {
            Iout[j+offset2]=Iout[j+offset2]/std::max(Icount[j],4.94e-324);
        }
    }
    free(Icount);
}
...
void interpolate_forward_3d_double(double *Iin, double *Tlocalx, double *Tlocaly, double *Tlocalz, int *Isize, int *ImageSize, double *Iout)
{
	...
	for(jz=0; jz<Isize[2]; jz++)
    {
		for(jy=0; jy<Isize[1]; jy++)
		{
			for(jx=0; jx<Isize[0]; jx++)
			{
				...
				jxmin=std::max(jx-1,0);
				jymin=std::max(jy-1,0);
				jzmin=std::max(jz-1,0);
				
				jxmax=std::min(jx+1,Isize[0]-1);
				jymax=std::min(jy+1,Isize[1]-1);
				jzmax=std::min(jz+1,Isize[2]-1);
				...
				b=(int)ceil(std::max(1.5*std::max(std::max(std::max(std::max(std::max(b1,b2),b3),b4),b5),b6),1.0));
	...
	for(j=0; j<nPixels2; j++)
    {
            Iout[j]=Iout[j]/std::max(Icount[j],4.94e-324);
    }
    free(Icount);
}
...
void interpolate_forward_3d_single(float *Iin, float *Tlocalx, float *Tlocaly, float *Tlocalz, int *Isize, int *ImageSize, float *Iout)
{
	...
	for(j=0; j<nPixels2; j++)
    {
            Iout[j]=Iout[j]/std::max(Icount[j],1e-40f);
    }
    free(Icount);
}